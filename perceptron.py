# python3中，reduce()函数已经在全局名字空间中被移除
...
class Perceptron(object):
    ...
    def one_iteration(self, input_vecs, labels, rate):
        ...
        一次迭代，把所有的训练数据过一遍
        '''
        # 把输入和输出打包在一起，成为样本的列表[(inputVec,label),....]
        # 而每个训练样本是(inputVec,label)
    ...
    def update_weights(self, input_vec, output, label, rate):
        ...
        # 然后利用感知器规则更新权重
        delta = label - output  # 误差
        # map函数返回的仅是map对象，必须列表化才行！！！找错误找着找着就秃了0.0，终于搞出来了
        self.weights = list(map(
            lambda x, w: w+rate*delta*x, input_vec, self.weights
        ))
